...
.PHONY: create_user
create_user:
	cd docker && docker-compose run -u dev workplace php bin/console app:create-user

.PHONY: seed
seed:
	cd docker && docker-compose run -u dev workplace php bin/console doctrine:fixtures:load --no-interaction
